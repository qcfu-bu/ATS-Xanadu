...
The server (`language-server/server/`) is written in ATS3 and compiled
...
the prelude).  See `../server/PLAN.md`.

## Run from source (F5, in-repo)

1. Build the server: `../server/tools/build.sh`
...
`../server/BUILD/`, `XATSHOME` as the repo root, the checker from
...
| Setting                   | Purpose                                                                                    |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| `ats3.server.path`        | Override the server binary (empty = auto-resolve from `server-dist/`, then `server/BUILD/`). |
