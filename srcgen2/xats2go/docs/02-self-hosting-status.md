...
- **Compiles ATS3 user programs byte-identically to the bundle**: all 12
  ...
  75/75.  Gate: `selfhost-build/build.sh gate`.
- **Compiles its OWN COMPLETE SOURCE byte-identically to the bundle**:
  ...
  `selfhost-build/build.sh sweep` — 193 PASS / 0 DIFF / 0 ERR, plus
...
- **Differential-first rule.**  A repro is only valid if it DIVERGES:
  ...
  the validated pattern; `build.sh probe` uses it by default.
- **Tiered harness** (`selfhost-build/build.sh`): `probe` ~3s;
