...
Status: **M2 IN PROGRESS** — M2.0–M2.5 ✅ (harness+liveness; scalars/primops native-typed; functions+TCO; if/let/case; **closures: inline Go func literals w/ lexical capture, `I1INSfix0` self-ref `var f;f=func…`, multi-level capture + shadowing sound**) · M2.6 (layout-aware tuples/records + `i1tnm→i0typ` side-table) **next — pivotal** · M0/M1 done · Go 1.26.4. **suite = 30/30 GREEN.**
