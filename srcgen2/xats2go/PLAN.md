...
Status: **M2 IN PROGRESS** — M2.0–M2.5 ✅; M2.6a ✅ (side-table); **M2.6b ✅ (value-typed tuples/records — flat→value `struct{...}`, boxed→`*struct`/`&struct`, per-level `trcdknd`; the core idiomatic-Go data payoff)** · M2.6c (lvalues/assignment) **next** · M0/M1 done · Go 1.26.4. **suite = 42/42 GREEN.**
