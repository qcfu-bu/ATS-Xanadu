...
(* ****** ****** *)
//
// xats2go MIGRATION: PASS-THROUGH the exception nodes (recursing into their
// sub-expressions like every other arm) so the tryd3i0 normalization pass does
// NOT clobber [I0Etry0]/[I0Eraise] into [I0Enone2] (its [_(*otherwise*)] arm).
// Without these, try/raise would survive trxd3i0 only to be erased here.
//
...
(* ****** ****** *)
//
// xats2go MIGRATION: tryd3i0 pass-through for [I0Etry0] (recurse into the
// normal body + the handler clauses), modeled on [f0_cas0].  Rebuilds the node
// identically so the tryd3i0 normalization keeps it intact.
//
fun
f0_try0
(
...
(
i0exp(loc0,
  i0t0, I0Etry0(tknd,i0e1,icls)))
//
end(*let*)//end-of-[f0_try0(i0e0,enw0)]
//
(* ****** ****** *)
//
// xats2go MIGRATION: tryd3i0 pass-through for [I0Eraise] (recurse into the
// exception expression), modeled on [f0_addr].
//
fun
f0_raise
(
