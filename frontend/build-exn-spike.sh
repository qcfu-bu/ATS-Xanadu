#!/usr/bin/env bash
########################################################################
# EXN GATING SPIKE — build + run the direct-L2 EXCEPTION-construction proof.
#
# Proves: a `D2Cdatatype` decl (exception MyExn(Int) + raise + try/with) + a function
...
#
# Usage:  bash frontend/build-exn-spike.sh
########################################################################
set -uo pipefail
...
DRV_DATS="$HERE/DATS/pyfront_exn_spike.dats"
GLUE="$HERE/CATS/pyfront_exn_spike.cats"
...
########################################################################
echo ">> [1/3] transpile the EXN spike driver (jsemit00)"
########################################################################
DRV_TRANS="$BUILD/pyfront_exn_spike_dats.js"
$NODE "$JSEMIT" "$DRV_DATS" > "$DRV_TRANS" 2>"$BUILD/exn-transpile.err"
echo "   transpiled driver -> $DRV_TRANS ($(wc -l < "$DRV_TRANS") lines)"
if [ "$(wc -l < "$DRV_TRANS")" -lt 5 ]; then
  echo "!! EXN driver transpile produced too little; see BUILD/exn-transpile.err" >&2
  tail -60 "$BUILD/exn-transpile.err" >&2; exit 1
fi
# a transpile-time errck (un-instantiated template / bad node) shows as F?PERR0-ERROR.
if grep -qE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/exn-transpile.err" 2>/dev/null; then
  echo "!! EXN driver transpile reported errck — see BUILD/exn-transpile.err" >&2
  grep -nE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/exn-transpile.err" | head -20 >&2
  exit 1
fi
...
########################################################################
OUT="$BUILD/exn-spike.out"
ERR="$BUILD/exn-spike.err"
...
if echo "$NERR_LINE" | grep -qE 'nerror \(after tread3a\) = 0$' && grep -q "RESULT: PASS" "$ERR"; then
  echo ">> EXN-SPIKE: PASS (direct-L2 exception decl + raise + try typecheck, nerror=0)"
  exit 0
else
  echo "!! EXN-SPIKE: FAIL (exn/raise/try did NOT typecheck nerror=0)" >&2
