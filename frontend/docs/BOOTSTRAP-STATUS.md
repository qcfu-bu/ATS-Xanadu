...
- The dynamic compiler/utility pyprint-only sweep now has a concrete baseline:
  ...
  `srcgen2/UTIL` files reports 38 visible `# TODO(pp)` markers. Printing ATS
  ...
  final RHS. The remaining clusters are inline dynamic expressions, local
...
- Dynamic pyprint breadth: drive the 166-file DATS/UTIL baseline from
  `TODOpp=38` to zero, starting with inline dynamic expressions and private/
