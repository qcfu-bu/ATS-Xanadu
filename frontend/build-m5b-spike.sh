#!/usr/bin/env bash
########################################################################
# M5b GATING SPIKE — build + run the direct-L2 DATATYPE-construction proof.
#
...
#
# Usage:  bash frontend/build-m5b-spike.sh
########################################################################
set -uo pipefail
...
DRV_DATS="$HERE/DATS/pyfront_m5b_spike.dats"
GLUE="$HERE/CATS/pyfront_m5b_spike.cats"
...
########################################################################
echo ">> [1/3] transpile the M5b spike driver (jsemit00)"
########################################################################
DRV_TRANS="$BUILD/pyfront_m5b_spike_dats.js"
$NODE "$JSEMIT" "$DRV_DATS" > "$DRV_TRANS" 2>"$BUILD/m5b-transpile.err"
echo "   transpiled driver -> $DRV_TRANS ($(wc -l < "$DRV_TRANS") lines)"
if [ "$(wc -l < "$DRV_TRANS")" -lt 5 ]; then
  echo "!! M5b driver transpile produced too little; see BUILD/m5b-transpile.err" >&2
  tail -60 "$BUILD/m5b-transpile.err" >&2; exit 1
fi
# a transpile-time errck (un-instantiated template / bad node) shows as F?PERR0-ERROR.
if grep -qE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/m5b-transpile.err" 2>/dev/null; then
  echo "!! M5b driver transpile reported errck — see BUILD/m5b-transpile.err" >&2
  grep -nE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/m5b-transpile.err" | head -20 >&2
  exit 1
fi
...
########################################################################
OUT="$BUILD/m5b-spike.out"
ERR="$BUILD/m5b-spike.err"
...
if echo "$NERR_LINE" | grep -qE 'nerror \(after tread3a\) = 0$' && grep -q "RESULT: PASS" "$ERR"; then
  echo ">> M5b-SPIKE: PASS (direct-L2 datatype decl + match typecheck, nerror=0)"
  exit 0
else
  echo "!! M5b-SPIKE: FAIL (datatype/match did NOT typecheck nerror=0)" >&2
