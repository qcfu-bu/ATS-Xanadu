#!/usr/bin/env bash
########################################################################
# SURF1 (implement + overload) GATING SPIKE — build + run the direct-L2 var/:= proof.
#
...
#
# Usage:  bash frontend/build-surf1-spike.sh
########################################################################
set -uo pipefail
...
DRV_DATS="$HERE/DATS/pyfront_surf1_spike.dats"
GLUE="$HERE/CATS/pyfront_surf1_spike.cats"
...
########################################################################
echo ">> [1/3] transpile the var spike driver (jsemit00)"
########################################################################
DRV_TRANS="$BUILD/pyfront_surf1_spike_dats.js"
$NODE "$JSEMIT" "$DRV_DATS" > "$DRV_TRANS" 2>"$BUILD/surf1-transpile.err"
echo "   transpiled driver -> $DRV_TRANS ($(wc -l < "$DRV_TRANS") lines)"
if [ "$(wc -l < "$DRV_TRANS")" -lt 5 ]; then
  echo "!! surf1 driver transpile produced too little; see BUILD/surf1-transpile.err" >&2
  tail -60 "$BUILD/surf1-transpile.err" >&2; exit 1
fi
# a transpile-time errck (un-instantiated template / bad node) shows as F?PERR0-ERROR.
if grep -qE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/surf1-transpile.err" 2>/dev/null; then
  echo "!! surf1 driver transpile reported errck — see BUILD/surf1-transpile.err" >&2
  grep -nE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/surf1-transpile.err" | head -20 >&2
  exit 1
fi
...
########################################################################
OUT="$BUILD/surf1-spike.out"
ERR="$BUILD/surf1-spike.err"
...
grep -E 'RESULT (case[34]|: )' "$ERR" | sed 's/^/   /'
if grep -q "RESULT: PASS" "$ERR"; then
  echo ">> SURF1-SPIKE: PASS (both var cases — straight-line + var-in-loop — typecheck, nerror=0)"
  exit 0
elif grep -q "RESULT: PARTIAL" "$ERR"; then
  echo "!! SURF1-SPIKE: PARTIAL (case1 GO, case2 var-in-loop NO-GO — see f3perr0 below)" >&2
  echo "--- f3perr0 diagnostics (stock reporter) ---" >&2
  grep -E 'F2PERR0-ERROR|F3PERR0-ERROR' "$ERR" | head -30 >&2
  exit 2
else
  echo "!! SURF1-SPIKE: FAIL (straight-line var did NOT typecheck nerror=0)" >&2
  echo "--- f3perr0 diagnostics (stock reporter) ---" >&2
  grep -E 'F2PERR0-ERROR|F3PERR0-ERROR' "$ERR" | head -30 >&2
  exit 1
fi
