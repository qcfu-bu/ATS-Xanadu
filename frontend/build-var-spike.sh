#!/usr/bin/env bash
########################################################################
# VAR (mutable cell) GATING SPIKE — build + run the direct-L2 var/:= proof.
#
...
#
# Usage:  bash frontend/build-var-spike.sh
########################################################################
set -uo pipefail
...
DRV_DATS="$HERE/DATS/pyfront_var_spike.dats"
GLUE="$HERE/CATS/pyfront_var_spike.cats"
...
########################################################################
DRV_TRANS="$BUILD/pyfront_var_spike_dats.js"
$NODE "$JSEMIT" "$DRV_DATS" > "$DRV_TRANS" 2>"$BUILD/var-transpile.err"
echo "   transpiled driver -> $DRV_TRANS ($(wc -l < "$DRV_TRANS") lines)"
if [ "$(wc -l < "$DRV_TRANS")" -lt 5 ]; then
  echo "!! var driver transpile produced too little; see BUILD/var-transpile.err" >&2
  tail -60 "$BUILD/var-transpile.err" >&2; exit 1
fi
# a transpile-time errck (un-instantiated template / bad node) shows as F?PERR0-ERROR.
if grep -qE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/var-transpile.err" 2>/dev/null; then
  echo "!! var driver transpile reported errck — see BUILD/var-transpile.err" >&2
  grep -nE "F2PERR0-ERROR|F3PERR0-ERROR" "$BUILD/var-transpile.err" | head -20 >&2
  exit 1
fi
...
########################################################################
OUT="$BUILD/var-spike.out"
ERR="$BUILD/var-spike.err"
...
grep -E 'RESULT (case[12]|: )' "$ERR" | sed 's/^/   /'
if grep -q "RESULT: PASS" "$ERR"; then
  echo ">> VAR-SPIKE: PASS (both var cases — straight-line + var-in-loop — typecheck, nerror=0)"
  exit 0
elif grep -q "RESULT: PARTIAL" "$ERR"; then
  echo "!! VAR-SPIKE: PARTIAL (case1 GO, case2 var-in-loop NO-GO — see f3perr0 below)" >&2
  echo "--- f3perr0 diagnostics (stock reporter) ---" >&2
  grep -E 'F2PERR0-ERROR|F3PERR0-ERROR' "$ERR" | head -30 >&2
  exit 2
else
  echo "!! VAR-SPIKE: FAIL (straight-line var did NOT typecheck nerror=0)" >&2
  echo "--- f3perr0 diagnostics (stock reporter) ---" >&2
  grep -E 'F2PERR0-ERROR|F3PERR0-ERROR' "$ERR" | head -30 >&2
  exit 1
fi
