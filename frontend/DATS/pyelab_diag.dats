...
| PCCexternval(_, _, _, _) => acc       // a bodyless dynamic constant — only a type, no poison nodes.
...
| PCCfixity(_, _, _, _) => acc   // FIXITY: a fixity decl carries only a kind/prec/names — no poison nodes.
