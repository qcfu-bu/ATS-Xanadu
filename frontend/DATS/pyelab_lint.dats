...
| PCEunit(_) => acc
| PCEerror(_, _) => acc
// A-TEMPLATE: `@inst[types] e` — walk its inner expr NON-tail (an instantiation of a loop self-call
// in tail position is exotic and not generated by the desugarer; non-tail is the safe default).
| PCEinst(_, _, e1) => lint_exp(e1, false, acc)
| PCEsapp(_, _, e1) => lint_exp(e1, false, acc)
// SCOPING: a `where:` body — the body expr inherits the where-expr's tail position (it IS the
// value); the where-decls are SEPARATE scopes, not part of THIS body's tail walk (their own loops
// are caught by the loop-finder lint_loops_exp).
| PCEwhere(_, body, _) => lint_exp(body, tail, acc)
...
// SCOPING: a `where:` body — find generated loops in BOTH the body AND every where-decl (a desugared
// loop can live inside a where-defined helper; lint_decls_go drives the per-decl loop walk).
| PCEwhere(_, body, ds) => lint_decls_go(ds, lint_loops_exp(body, acc))
| _ => acc
)
//
and
...
| PCCimplement(_, _, _, _, _, _, _, body, _) => lint_loops_exp(body, acc) // an implement body is a fun body — lint its loops.
...
// SCOPING: a `private` run — lint the loops in every private decl it carries.
| PCCprivate(_, ds) => lint_decls_go(ds, acc)
| _ => acc
)
//
and
